...
Firebug.FirepickerModel = extend(Firebug.Module, {
  ...
  hookIntoColorInfoTip: function() {
    var self = this, stylesheetPanel = Firebug.getPanelType('css'), original = stylesheetPanel.prototype.showInfoTip, self = this;
    setTimeout(function() { self.log(stylesheetPanel); }, 6000);
    stylesheetPanel.prototype.showInfoTip = function(infoTip, target, x, y) {
      var infoTipTypeBefore = this.infoTipType, origResult = original.call(this, infoTip, target, x, y);
      if (origResult && this.infoTipType == 'color' && infoTipTypeBefore != this.infoTipType) {
        try {
          var valueElement = getAncestorByClass(target, "cssPropValue");
          if (valueElement)
          {
            var offset = getClientOffset(valueElement);
            var offsetX = x-offset.x;

            var text = valueElement.textContent;
            var charWidth = valueElement.offsetWidth/text.length;
            var charOffset = Math.floor(offsetX/charWidth);

            var cssValue = parseCSSValue(text, charOffset);
            var obj = {
              'valueElement': '<span class="cssPropValue editable ">',
              'offset': 'Object x=76 y=152',
              'offsetX': '5',
              'text': '#333333',
              'charWidth': '6.571428571428571',
              'charOffset': '0',
              'cssValue': 'Object value=#333333 start=0 end=6 type=rgb'
            }
            self.log(valueElement);
            
            var panel = $('fp-panel', document), deck = $('fbPanelBar2', document).deck,
                browser = $('fp-panel-browser', document), body = browser.contentDocument.body;
            // var pos = this.getPosition(propValueBox);
            
            // panel.openPopup(deck, "overlap", 250, 0, false, true);
            // self.log(body.clientWidth);
            // self.log(body.querySelector('div.panelNode').clientWidth);
            // setTimeout(function(){
            //   self.log(body.clientWidth);
            //   self.log(body.querySelector('div.panelNode').clientWidth);
            // }, 5000);
            // self.log(panel.sizeTo);
            // self.log(browser.sizeTo);
            // setTimeout(function() {
            //   
            // }, 0);
            // setTimeout(function() {
            //   panel.sizeTo(160, 160);
            //   // browser.resizeTo(100, 250);
            //   // browser.setAttribute('width', body.clientWidth);
            //   // browser.setAttribute('height', body.clientHeight);
            //   browser.setAttribute('width', 250);
            //   browser.setAttribute('height', 200);
            // }, 2500);
            // self.log(body);
            
            // var dialog = self.getPickerDialogFrom(infoTip.parentNode, cssValue.value);
            // dialog.style.left = x + 'px';
            // dialog.style.top  = y + 'px';
            // self.log(dialog);
            // dialog.setAttribute('active', 'true');
            // self.addStyleSheet(infoTip.ownerDocument);
            // self.log('propValue', propValue, 'offset', offset, 'offsetX', offsetX, 'text', text, 'charWidth', charWidth, 'charOffset', charOffset, 'cssValue', cssValue);
          }
        } catch(e) {
          self.log(e);
        }
        self.log(this.infoTipObject);
      }
      return origResult;
    }
  },
  
  setNewValue: function(valueElement, newValue) {
    var panel = Firebug.getElementPanel(valueElement), editor = panel.getEditor(valueElement);
    editor.panel = panel;
    editor.saveEdit(valueElement, newValue);
  },
  
  hookIntoInfoTip: function() {
    var calls = [];
    var original = Firebug.InfoTip.populateColorInfoTip, self = this;
    Firebug.InfoTip.populateColorInfoTip = function(infoTip, color) {
      // self.log(color);
      return original.call(this, infoTip, color);
    }
  },
